...
| `conscience.py` — many engines, one life (2026-09-23) | Claude Desktop keeps its JEFFEREY process alive while the owner talks to Hermes or a GPT, each holding the store in memory. Before this, one engine's write made every other one stale — serving old facts, and its next write refused as a conflict until the app was quit. Now every surface calls `refresh()` before each tool call: one `stat` (inode + mtime + size, right on 1-second-mtime disks) and a re-read only when another process wrote. The anti-clobber guard is unchanged underneath. Selftest uses a **real second process**, and was confirmed to fail with the old behaviour |
